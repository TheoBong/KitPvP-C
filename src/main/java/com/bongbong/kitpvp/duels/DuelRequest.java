...
import com.bongbong.kitpvp.duels.impl.Duel;
    ...
    public void start() {
        ...
        if(to != null && from != null && !isExpired()) {
            ...
            if(toProfile != null && fromProfile != null && toProfile.getState().equals(PlayerState.SPAWN) && fromProfile.getState().equals(PlayerState.SPAWN)) {
                Duel duel = new Duel(plugin, RandomStringUtils.randomAlphanumeric(8).toUpperCase());
                duel.join(to);
                duel.join(from);
                duel.setKit(kit);
                duel.setArena(arena);
                duel.start();
                plugin.getDuelsOccupationManager().getOccupations().put(duel.getId(), duel);
